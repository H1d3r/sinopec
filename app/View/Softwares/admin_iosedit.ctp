  <div class="call">
   <?php echo $this->Form->create('Software',array('onSubmit'=>'return funcChina();','div'=>false,'name'=>'frm','enctype' => 'multipart/form-data','action'=>'pmsave')); ?>
								...
								<?php if($Softlist[0]['softwares']['parent_id']!=0){?>	
                                    ...
                                    <tr>
                                            <td class="call_1">上传IPA：</td>
                                            <td class="call_3"><input type="file" id="images_url" name="data[Software][images_url][]">如果不修改则留空 (支持IPA)
											</td>
                                    </tr>  
									<tr>
                                            <td class="call_1">上传PLIST：</td>
                                            <td class="call_3"><input type="file" id="plist" name="data[Software][plist]">如果不修改则留空 (支持PLIST)
											</td>
                                    </tr>      									
                                    <tr>
					...
					<?php echo $this->Form->input('typeipa',array('label'=>false,'type'=>'hidden','div'=>false,'value'=>'1'))?>
		...
		if($('#isCheck').val()==0){
			$('#div_text').toggle();
				...
				function funcChina(){
						...
						var obj = $('#images_url').val(); 
						if(obj!=""){
							if(/.*[\u4e00-\u9fa5]+.*$/.test(obj)){ 
									alert("上传IPA文件不能以汉字命名！"); 
									return false; 
								} 
							}
						var obj1 = $('#plist').val(); 
